...
/** turns the set into a vector
  ...
  /** reads the contents of both dictionary and the file that 
  contains the incorrect words 
  @post if successful, the content of either dictionary or 
  incorrect words is copied to a vector
  @return returns a vector that contains words from either dictionary or 
  incorrect words
  @param filename the name of the file which content will be extracted*/
  std::vector <std::string> read(std::string filename);

  /** reads the contents of both dictionary and the file that 
  contains the incorrect words 
  @post if successful, the misspelled words are added to differenceSet and returned
  @return returns a set that contains misspelled words
  @param dictVect incorrectVect contain contents of dictionary.txt and incorrect.txt */
  ArraySet<std::string> check(std::vector<std::string> dictVect, std::vector<std::string> incorrectVect);
